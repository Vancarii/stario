...
class JustAudioPlayer implements AudioPlayerService {
  ...
  @override
  Stream<AudioProcessingState> get audioProcessingState => _audioPlayer.playerStateStream.map(
        (_playerStateMap),
      );

  @override
  Stream<List<SongModel>> get currentPlaylist => _audioPlayer.sequenceStateStream.map(
        (sequenceState) {
  ...
  @override
  Stream<PlaylistLoopMode> get loopMode => _audioPlayer.loopModeStream.map((_loopModeMap));
  ...
  Future<void> seekToStart() {
    return _audioPlayer.seek(Duration.zero, index: _audioPlayer.effectiveIndices?.first);
  }
  ...
  @override
  Future<Duration> loadPlaylist(List<SongModel> playlist) {
